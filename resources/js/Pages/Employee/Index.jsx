    ...
    const renderPagination = () => {
        ...
        for (let i = Math.max(1, currentPage - 1); i <= Math.min(lastPage, currentPage + 1); i++) {
            ...
            paginationLinks.push(
                ...
                <button
                    ...
                    onClick={() => window.location.assign(employees.links[i].url)} // เมื่อคลิกปุ่ม ให้เปลี่ยนหน้าไปยัง URL ของหน้านั้น
                >
                    {i}
                </button>
            );
        ...
        if (currentPage < lastPage - 2) {
            paginationLinks.push(<span key="ellipsis2"
                className='relative inline-flex items-center px-4 py-2 text-sm font-semibold text-gray-700 ring-1 ring-inset ring-gray-300 focus:outline-offset-0'>...</span>);
            paginationLinks.push(
                <button key={lastPage} className="relative inline-flex items-center px-4 py-2 text-sm font-semibold text-gray-700 ring-1 ring-inset ring-gray-300 focus:outline-offset-0" onClick={() => window.location.assign(employees.links[lastPage].url)}>
                    {lastPage}
                </button>
            );
        }
        ...
        if (currentPage < lastPage) {
            paginationLinks.push(
                <button key="next" className="relative inline-flex items-center rounded-r-md px-2 py-2 text-gray-400 ring-1 ring-inset ring-gray-300 hover:bg-gray-50 focus:z-20 focus:outline-offset-0" onClick={() => window.location.assign(employees.next_page_url)}>
                    Next
                </button>
            );
        }
